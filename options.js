...
var DEFAULT_IMAGE_URLS = [
  '//storage.googleapis.com/customGCal/img.jpg',
];
...
function loadImageURLOption(singleImageURL) {
  setTab('t1');
...
/** Show single tab, and update the one URL to use. */
function loadImageFileOption(singleImageFile) {
  setTab('t3');
  document.getElementById('fileInput') = singleImageFile;
  updateImageUrl(singleImageFile);
}
...
function loadOptions() {
  ...
  chrome.storage.sync.get({
    ...
    if (parsedColorHex) {
      ...
      loadImageURLOption(items.imageURL);
    }
  });
}

/** Save a single image URL as the one to use. */
function saveImageURLOption() {
...
/** Save a single image file as the one to use. */
function saveImageFileOption() {
  ...
  if (fileInput.files.length > 0) {
    ...
    reader.onload = function(event) {
        ...
        // Store the data URL in persistent local storage
        chrome.storage.sync.set({
